...
function shift_stacks_ordered(command, stacks){
    ...
    for(let i=0; i<num; i++){
        pop_tmp = stacks[from_stack].pop();
        pop_arr.push(pop_tmp[0]);
    }

    for(let i=0; i<num; i++){
        pop_tmp = pop_arr.pop();
        stacks[to_stack].push(pop_tmp[0]);
    }
...
for(let i=row_num; i<input.length; i++){
    ...
    if(input[i][0] == 'm'){
        stacks = shift_stacks_ordered(input[i], stacks);
    }
    // console.log(stacks);
}
// console.log(stacks)
