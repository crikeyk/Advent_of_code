...
get_shortest_path(end_pos, 0)

let min_trail = max_dist

for(let i=0; i<input.length;i++){
    for(let j=0; j<input[0].length;j++){
        if(input[i][j] == 'a' && shortest_path_map[i][j] < min_trail){
            min_trail = shortest_path_map[i][j]
        }
    }
}

console.log("shortest trail is ", min_trail)
